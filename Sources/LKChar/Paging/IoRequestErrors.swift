...
class IoRequestErrors: UITableViewCell {
    ...
    @IBOutlet weak var eParseFloat: UIButton!
    ...
    @IBAction func onTapGesture(_ sender: Any) {
        ...
        iBoundaryString.fPositionRight = oBundledPath.vAllSeries!
        if oBundledPath.xImgDefinition.count != 0 {
            iBoundaryString.iGetFacets = oBundledPath.xImgDefinition
        }else{
            iBoundaryString.iGetFacets = oBundledPath.cServicePackage!
        }
        iBoundaryString.vBuilderBuffer = oBundledPath.vBuilderBuffer
    ...
    func addLocalMonitorForEvents(_ models : IoComponentViewOnce) {
        oBundledPath = models
        eParseFloat.getQuakeData("", oBundledPath.vAllSeries!)
    ...
    override func awakeFromNib() {
        ...
        eParseFloat.frame = CGRectMake(tEnableAttitude.right + 8, 0, 116, 180)
        eParseFloat.imageView?.contentMode = .scaleAspectFill
        tEnableAttitude.imageView?.contentMode = .scaleAspectFill
        eParseFloat.imageEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
                
        objectToCopy(#selector(backFileData(_ :)), "habido_uidevice_mipsel")
        ...
        jPartitionBy.addGestureRecognizer(kSharpFolder)
                
                
        self.contentView.addSubview(fConnectIndex)
        fConnectIndex.snp.makeConstraints { make in
            make.centerY.equalTo(self.eParseFloat)
            make.right.equalTo(self.eParseFloat.snp.left).offset(-10)
        }
    ...
    @objc func backFileData(_ noty : NSNotification) {
        let vTokenizeString = noty.object as! NSDictionary
        eSuggestedFilename = vTokenizeString.setTimeInSeconds(oBundledPath.gAvatarChange)
    }
    ...
    override func layoutSubviews() {
        ...
        if oBundledPath.kDelegateModal!.count > 0 {
            jPartitionBy.y = 27
        }else{
            jPartitionBy.y = 0
        }
        if oBundledPath.vBuilderBuffer == true {
            ...
            eParseFloat.x = tEnableAttitude.x - 124
        }else{
            tEnableAttitude.x = 16
            eParseFloat.x = tEnableAttitude.right + 8
        }
        pMatchingDecimal.frame = CGRectMake(eParseFloat.width/2 - 20 + eParseFloat.x, eParseFloat.height/2 - 20, 40, 40)
        pMatchingDecimal.center = eParseFloat.center
        lMockPosition.frame = CGRectMake(0, 0, contentView.width, 20)
                
        if yOwnsBuffer == false {
            eParseFloat.y = jPartitionBy.y
            qDotType.text = ""
        }else {
            eParseFloat.y = 16 + jPartitionBy.y
        }
